...
export type FrontRequestHandler = (
  args: FrontRequestHandlerArgs,
) => Promise<void>
export const frontRequestHandler =
  (sendMessage: SendMessage): FrontRequestHandler =>
  async ({ command, messages }: FrontRequestHandlerArgs): Promise<void> => {
    return command().then(
      async (res) => {
        ...
        if (400 <= res.status) {
          await res
            .json()
            .then((body: FrontErrorResponseBody | null) => {
              const message = body
                ? messages.errorResponse(body.code)
