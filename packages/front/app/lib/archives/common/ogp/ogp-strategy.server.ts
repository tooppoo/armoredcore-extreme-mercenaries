import { youtubePattern, youtubeWithQueryPattern } from '~/lib/archives/common/url/support-url.server'
...
export type GetOGPStrategy = (url: URL, strategies: OGPStrategy[]) => OGPStrategy
...
export const withOGPScanner = (cond: OGPStrategyCondition = () => true): OGPStrategy => defineStrategy(
  'withOGPScanner',
  cond,
  async (url) => {
...
export const withYouTubeData = (): OGPStrategy => defineStrategy(
  'withYouTubeData',
  (url) => youtubePattern.test(url.toString()) || youtubeWithQueryPattern.test(url.toString()),
  (() => {
...
function defineStrategy(
  ...
  return {
    name,
    condition,
    run: (url, env) => run(url, env).catch((error) => {
      throw {
        code: failedGetOGP,
