...
export function validateVideoCommand(options?: Option[]): Validation<{
  ...
  if (!isNonEmpty(url)) {
    return { ok: false, code: 'missing_required_field', message: messageFor('missing_required_field') }
  }
  try {
    new URL(String(url))
  } catch {
    return { ok: false, code: 'invalid_url', message: messageFor('invalid_url') }
  }
  return { ok: true, data: { url: String(url), title: title as string | undefined, description: description as string | undefined } }
}
...
> {
  ...
  if (isNonEmpty(url)) {
    ...
    return { ok: true, data: { kind: 'link', title: String(title), url: String(url), description: description as string | undefined } }
  }
