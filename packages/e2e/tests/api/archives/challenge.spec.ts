...
test('upload link', async ({ page, request }) => {
  ...
  await expect(
    page.locator(
      'a[href="https://x.com/RLF_Officer/status/1886619872047898811"]',
    ),
  ).toHaveCount(1)
})
...
test('(youtu.be) invalid url', async ({ request }) => {
  test.slow()
...
test('(youtube) invalid url', async ({ request }) => {
  test.slow()
...
test('(example.com) invalid url', async ({ request }) => {
  test.slow()
