...
setup('setup db', async () => {
  ...
  try {
    ...
    execSync(`${frontCommand} -- sql:test -- --command ".tables"`, { stdio: 'inherit' })
    ...
    try {
      ...
      console.log('Cleanup failed (tables might not exist yet), continuing with seeding...')
    ...
    for (const file of seedFiles) {
      ...
      execSync(`${frontCommand} -- sql:test -- --file ${seedPath}`, { stdio: 'inherit' })
    }
    
    // Verify data was inserted
    console.log('Verifying seed data...')
    execSync(`${frontCommand} -- sql:test -- --command "SELECT COUNT(*) as video_count FROM video_archives;"`, { stdio: 'inherit' })
    execSync(`${frontCommand} -- sql:test -- --command "SELECT COUNT(*) as challenge_count FROM challenge_archives;"`, { stdio: 'inherit' })
